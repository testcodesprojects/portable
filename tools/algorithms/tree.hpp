/**
 * @file tree.hpp
 * @brief Tree structures and utilities for hierarchical task scheduling.
 *
 * This file implements tree-based data structures for managing dependencies and
 * scheduling in hierarchical computations. Key functionalities include:
 * - Binary tree construction for GEMM operation management
 * - Reduced tree structures optimized for sparse configurations
 * - Memory management and tree traversal utilities
 * - GEMM operation counting and task scheduling support
 *
...
// #include "sparse_utils.hpp"  // File removed - not needed
#include "../memory/MemoryManager.hpp"
#include "../common/stiles_logger.hpp"
#include "../TileIndexer/TileIndexerUtils.hpp"
...
TreeLeaf* createOptimizedTree(int num_splits, int num_tasks, int leafheight, int leafwidth, int group_index) {
    ...
    for (int i = 0; i < num_splits; ++i) {
        ...
        if (!tree->nodes[i].x) {
            ...
            for (int j = 0; j < i; ++j) {
                MemoryManager::deallocate(tree->nodes[j].x);
            }
            MemoryManager::deallocate(tree->dependency);
            MemoryManager::deallocate(tree->nodes);
            MemoryManager::deallocate(tree);
            return nullptr;
        }
        ...
        tree->nodes[i].dirty = 0;  // clean scratch — no contributions yet
    }
    ...
    while (k < total_num_used_tiles && m < total_num_used_tiles) {
        ...
        if (m == k) {
            if (n != k) {
                ...
                if (tilecounter::checkBitNeighbor(bit_array, n, k, total_num_used_tiles)){
            ...
            if (n != k) {
                ...
                if(tilecounter::checkBitNeighbor(bit_array, n, k, total_num_used_tiles) && tilecounter::checkBitNeighbor(bit_array, n, m, total_num_used_tiles)){ 
    ...
    while (k < total_num_used_tiles && m < total_num_used_tiles) {
        ...
        if (m == k) {
            if (n != k) {
                ...
                if(tilecounter::isNeighbor(n, k, neighbors, neighbors_sizes)){ 
            ...
            if (n != k) {
                ...
                if(tilecounter::isNeighbor(n, k, neighbors, neighbors_sizes) && tilecounter::isNeighbor(n, m, neighbors, neighbors_sizes)){ 
...
int TREE_SETUP_STG2_STILES_PHASE_1(int num_tiles, int tile_size, int *tree_sep, bool *on_off_tiles, int** gemmcounter_redtree, int acc, TreeLeaf ***scheme_trees, int CORE_SIZE, int group_index){
    ...
    int num_trees = 0;
    TreeLeaf **trees = MemoryManager::allocateZero<TreeLeaf*>(num_sep, group_index);
    ...
    if (trees == NULL) {
        ...
        MemoryManager::deallocate(*gemmcounter_redtree); 
    ...
    if(num_trees==0){
        ...
        MemoryManager::deallocate(trees);  // Free the allocated array if no trees were created.
        *tree_sep = 0;
