/**
 * @file AlgorithmsMemoryManager.hpp
 * @brief Memory manager for algorithm-specific data structures.
 *
 * Handles memory allocation for task arrays, work buffers, and other
 * temporary storage used during algorithm execution.
 *
...
#ifndef STILES_ALGORITHMS_MEMORY_MANAGER_HPP
#define STILES_ALGORITHMS_MEMORY_MANAGER_HPP
...
struct AlgorithmsAllocationRecord {
    void* ptr;
    size_t size;
    int groupID; // Use -1 if not assigned.
};

class AlgorithmsMemoryManager {
public:
    ...
    static void debugDump(std::ostream& out = std::cout) {
        ...
        std::map<int, std::vector<AlgorithmsAllocationRecord>> grouped;
        for (const auto& record : globalTracker) {
...
private:
    ...
    static void untrack(void* ptr) {
        std::lock_guard<std::mutex> lock(globalTrackerMutex);
        auto it = std::find_if(globalTracker.begin(), globalTracker.end(), [ptr](const AlgorithmsAllocationRecord &rec) {
            return rec.ptr == ptr;
        });
    ...
    inline static std::vector<AlgorithmsAllocationRecord> globalTracker;
    inline static std::mutex globalTrackerMutex;
};

#endif // STILES_ALGORITHMS_MEMORY_MANAGER_HPP
