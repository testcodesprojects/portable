/**
 * @file OrderingMemoryManager.hpp
 * @brief Memory manager for matrix ordering operations.
 *
 * Manages memory allocation for permutation arrays, graph structures,
 * and temporary buffers used during matrix reordering.
 *
...
#ifndef STILES_ORDERING_MEMORY_MANAGER_HPP
#define STILES_ORDERING_MEMORY_MANAGER_HPP
...
public:
    ...
    static void debugDump(std::ostream& out = std::cout) {
        ...
        out << "===== OrderingMemoryManager Allocation Summary =====\n";

        std::map<int, std::vector<OrderingAllocationRecord>> grouped;
        for (const auto& record : globalTracker) {
...
private:
    ...
    static void untrack(void* ptr) {
        ...
        auto it = std::find_if(globalTracker.begin(), globalTracker.end(),
                               [ptr](const OrderingAllocationRecord& rec) {
                                   return rec.ptr == ptr;
                               });
    ...
    inline static std::vector<OrderingAllocationRecord> globalTracker;
    inline static std::mutex globalTrackerMutex;
};

#endif // STILES_ORDERING_MEMORY_MANAGER_HPP
