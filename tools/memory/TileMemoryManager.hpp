/**
 * @file TileMemoryManager.hpp
 * @brief Memory manager specialized for tile data structures.
 *
 * Handles allocation and deallocation of memory for dense and sparse tiles
 * used in tiled matrix computations, with group-based organization.
 *
...
#ifndef STILES_TILE_MEMORY_MANAGER_HPP
#define STILES_TILE_MEMORY_MANAGER_HPP
...
public:
    ...
    static T* allocate(size_t count, int groupID = -1) {
        ...
        if (count > std::numeric_limits<size_t>::max() / sizeof(T)) {
            reportError("Allocation size overflow.");
            return nullptr;
        }
        size_t bytes = count * sizeof(T);
        void* raw = sTiles_malloc_aligned(bytes, STILES_ALIGN);
        if (!raw) {
    ...
    static T* allocateZero(size_t count, int groupID = -1) {
        if (count == 0) return nullptr;
        if (count > std::numeric_limits<size_t>::max() / sizeof(T)) {
            reportError("Zeroed allocation size overflow.");
            return nullptr;
        }
        size_t bytes = count * sizeof(T);
        void* raw = sTiles_calloc_aligned(count, sizeof(T), STILES_ALIGN);
        if (!raw) {
            reportError("Zeroed memory allocation failed.");
            return nullptr;
        }
        T* ptr = static_cast<T*>(raw);
        track(ptr, bytes, groupID);
    ...
    static void freeAll() {
        ...
        for (const auto& record : globalTracker) {
            sTiles_free_aligned(record.ptr);
        }
        globalTracker.clear();
    }
    ...
    static void freeAllGroup(int groupID) {
        ...
        for (int i = static_cast<int>(globalTracker.size()) - 1; i >= 0; --i) {
            if (globalTracker[i].groupID == groupID) {
                sTiles_free_aligned(globalTracker[i].ptr);
    ...
    static void debugDump(std::ostream& out = std::cout) {
        ...
        out << "===== TileMemoryManager Allocation Summary =====\n";

        std::map<int, std::vector<TileAllocationRecord>> grouped;
        for (const auto& record : globalTracker) {
        ...
        for (const auto& entry : grouped) {
            ...
            for (const auto& record : records) {
                out << "  [" << record.ptr << "] Size: " << std::setw(8) << record.size << " B\n";
...
private:
    ...
    static void untrack(void* ptr) {
        std::lock_guard<std::mutex> lock(globalTrackerMutex);
        auto it = std::find_if(globalTracker.begin(), globalTracker.end(), [ptr](const TileAllocationRecord& rec) { return rec.ptr == ptr; });
    ...
    inline static std::vector<TileAllocationRecord> globalTracker;
    inline static std::mutex globalTrackerMutex;
};

#endif // STILES_TILE_MEMORY_MANAGER_HPP
...
//                                [ptr](const TileAllocationRecord& rec) {
//                                    return rec.ptr == ptr;
//                                });
//         if (it != globalTracker.end()) {
//             globalTracker.erase(it);
...
//                             int line = __LINE__) {
//         std::cerr << "TileMemoryManager Error: " << message
//                   << " (File: " << file << ", Line: " << line << ")\n";
//         throw std::runtime_error(message);
//     }
